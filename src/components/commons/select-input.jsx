...
const SelectInput = (props) => {
    ...
    const handleClick = (e) => {
        props.onSelect ? dispatcher(props.action(props.onSelect(e))) : props.triggerChange(e);
    ...
    return (
        <div className="relative w-full">
            ...
            <div id='drop-list' className={`${!showDropdown && 'hidden'} absolute z-50 max-h-40 overflow-x-hidden overflow-y-auto sc py-2 flex flex-col w-full rounded-md bg-white dark:bg-darkTheme4 shadow-xl dark:shadow-none dark:border-[1px] dark:border-t-0 dark:border-darkThemeTextLight`}>
                {
                    props.list.map((item, index) => (<p key={index} className='px-4 py-1 cursor-pointer hover:bg-lightBlue hover:dark:bg-darkTheme1 text-darkThemeTextLight text-[16px]' onClick={handleClick}>{item}</p>))
                }
            </div>
        </div>
    );

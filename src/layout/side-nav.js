			...
			<div className='flex-col justify-center items-center space-y-8 w-full'>
				{
					menuList.map((item, index) => (
						<NavLink key={index} className={`flex items-center ${isCollapsed ? 'justify-center space-x-0 px-0' : 'justify-start space-x-4 px-2'} text-base py-3 text-greyFontColor dark:text-darkThemeTextLight hover:text-primaryBlue ${location.pathname.includes(item.path) ? 'active' : ''}`} to={`/${item.path}`}>
							{item.icon}
